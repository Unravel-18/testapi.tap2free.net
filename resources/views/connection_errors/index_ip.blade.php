    ...
    <table class="table table-striped table-bordered tbl-servers">
        ...
        <thead>
			<tr>
				...
				<th style="vertical-align: middle;" colspan="1"></th>
			</tr>
		</thead>
		...
		<tbody>
			@foreach ($connection_errors as $connection_error)
                ...
                <tr class="tr-server @if($connection_error->status == '0') tr-bg-active @endif" data-id="{{{ $connection_error->id }}}">
                    ...
                    <td style="text-align: center;"><button data-href="{{ route('apis.connection_errors.destroy_ip_country', ['connection_error_id' => $connection_error->id]) }}" class='btn btn-sm btn-danger glyphicon glyphicon-trash btn-click-confirm' title="Удалить" type="button"></button></td>
                </tr>
			@endforeach
		</tbody>
	</table>
...
function deleteItems(obj) {
    ...
    if(items_id.length && confirm('Вы действительно хотите удалить выбранные Читы ?')){
            $.ajax({
                headers: { 'X-CSRF-TOKEN': window.Laravel.csrfToken },
		    	url: "{{ route('apis.connection_errors.delete_ip_country') }}",
		    	data: {
		    	    items_id: items_id,
