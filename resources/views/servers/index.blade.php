...
<div class="row" style="margin: 12px;">
  <div class="col-sm-12">
    ...
    <div style="padding-bottom: 24px;">
      ...
      <tr>
      <td>
      <a onclick="clickBtnAddServer(this)" class="btn btn-default btn-sm" href="{{ route('servers.add', []) }}" style="float: left;">Add</a>
      </td>
      <td>
      ...
      <td>
      <button class="btn btn-default btn-sm" id="update_crt_sel" style="float: left;">
        Копировать
    ...
    <table class="table table-striped table-bordered tbl-servers">
		...
		<thead>
			<tr>
				...
				<th style="vertical-align: middle;"><a onclick="changeSortField(this)" data-name="sort" data-value="{{ $sort == 'auto_switch_on_date' ? '-' : '' }}auto_switch_on_date" href="?sort={{ $sort == 'auto_switch_on_date' ? '-' : '' }}auto_switch_on_date">Auto<br />switch<br />date</a></th>
				<th style="vertical-align: middle;" colspan="3"></th>
			</tr>
		</thead>
		...
		<tbody>
			@foreach ($servers as $server)
                ...
                <tr class="tr-server @if($server->status == '0') tr-bg-active @endif" data-id="{{{ $server->id }}}">
					<td><input type="checkbox" name="select_servers[]" class="select_servers" data-id="{{{ $server->id }}}" /></td>
                    <td style="white-space: nowrap;" >
                      <span data-id="{{{ $server->id }}}" data-type="up" onclick="clickDisplaceServer(this)" style="cursor: pointer;">
                        &nbsp;<i class="fa fa-long-arrow-up" aria-hidden="true"></i>&nbsp;
                      </span>
                      <span>
                        &nbsp;
                      </span>
                      <span data-id="{{{ $server->id }}}" data-type="down"  onclick="clickDisplaceServer(this)" style="cursor: pointer;">
                    ...
                    <td style="text-align: center;"><a class="btn btn-info btn-sm glyphicon glyphicon-cog" title="Изменить" href="{{ isset($api) ? route('apis.servers.edit', ['server_id' => $server->id, 'api_id' => $api->id]) : route('servers.edit', ['server_id' => $server->id]) }}"></a></td>
                    <td style="text-align: center;"><a class="btn btn-success btn-sm glyphicon glyphicon-list" title="Апи" href="{{ isset($api) ? route('apis.servers.apis', ['server_id' => $server->id, 'api_id' => $api->id]) : route('servers.apis', ['server_id' => $server->id]) }}"></a></td>
                    <td style="text-align: center;"><button data-href="{{ isset($api) ? route('apis.servers.destroy', ['server_id' => $server->id, 'api_id' => $api->id]) : route('servers.destroy', ['server_id' => $server->id]) }}" class='btn btn-sm btn-danger glyphicon glyphicon-trash btn-click-confirm' title="Удалить" type="button"></button></td>
                </tr>
			@endforeach
		</tbody>
	</table>
