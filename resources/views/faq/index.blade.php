...
<div class="row" style="margin: 0px 6px 6px 6px;">
  <div class="col-sm-12">
    <h2>Вопросы Апи: <span style="color: #20B2AA;">{{ strtoupper($api->name) }}</span></h2>
  </div>
</div>
@endsection
...
<div class="row">
  <div class="col-sm-12">
    ...
    <div style="padding-bottom: 24px;">
      <a onclick="clickBtnAddServer(this)" class="btn btn-default btn-sm" href="{{ route('apis.faq.add', ['api_id' => $api->id]) }}" style="float: left;">Добавить</a>
      <a class="btn btn-default btn-sm" href="{{ route('apis.languages.index', ['api_id' => $api->id]) }}" style="float: left;">Языки</a>
    </div>
		...
		<tbody>
			@foreach ($items as $item)
				<tr class="tr-server" data-id="{{{ $item->id }}}">
                    ...
                    <td><a style="color: black;" title="Перейти" href="{{ route('apis.faq.edit', ['faq_id' => $item->id, 'api_id' => $api->id]) }}">{{{ $item->question }}}</a></td>
                    <td>{{{ str_limit($item->answer, 100) }}}</td>
					
                    <td style="text-align: center;"><a class="btn btn-info btn-sm glyphicon glyphicon-cog" title="Изменить" href="{{ route('apis.faq.edit', ['faq_id' => $item->id, 'api_id' => $api->id]) }}"></a></td>
					<td style="text-align: center;"><a class="btn btn-success btn-sm glyphicon glyphicon-list" title="Апи" href="{{ isset($api) ? route('apis.faq.apis', ['faq_id' => $item->id, 'api_id' => $api->id]) : route('servers.apis', ['faq_id' => $item->id]) }}"></a></td>
                    <td style="text-align: center;">
						<form method="POST" action="{{ route('apis.faq.destroyapi', ['faq_id' => $item->id, 'api_id' => $api->id]) }}">
                            {{ csrf_field() }}
                            <input name="_method" type="hidden" value="DELETE" />                        
                            <button class='btn btn-sm btn-danger glyphicon glyphicon-remove btn-delete' title="Удалить" type="button"></button>
                        </form>
					</td>
				</tr>
			@endforeach
...
function clickBtnAddServer(obj) {
    ...
    if (select_id) {
        $.ajax({
		    	...
		    	url: "{{route('faq.save_select_checked', ['api_id' => $api->id])}}",
		    	data: {
		    	    select_id: select_id,          
                },
                type: 'POST',
...
function clickDisplaceServer(obj) {
    ...
    $.ajax({
                headers: { 'X-CSRF-TOKEN': window.Laravel.csrfToken },
		    	url: "{{route('faq.displace_sort', ['api_id' => $api->id])}}",
		    	data: {
