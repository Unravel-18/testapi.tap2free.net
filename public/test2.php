...
$globaloptions[CURLOPT_HTTPHEADER] = array(
    "app-id:" . Helper::crypt("fw5r243fdzfhge24yh6nserehjfhmj7i6", 2, "2ehmcw30h0tewn1otz2"),
);
...
if ($chinfo['http_code'] == 200 && $json) {
    ...
    if ($json && isset($json[0])) {
        ...
        $ch = curl_init();
        $options = $globaloptions;
        $options[CURLOPT_URL] = "https://testapi.tap2free.net/api/main-server?ip=" . Helper::crypt($json[0]["ip"], 2, "2ehmcw30h0tewn1otz2");
        curl_setopt_array($ch, $options);
        $response = curl_exec($ch);
        $chinfo = curl_getinfo($ch);
        curl_close($ch);
        ...
        if ($json) {
            if (!stripos($json['config'], "\n")) {
                $json['config'] = Helper::synchron_decode($json['config'], "2ehmcw30h0tewn1otz2");
            }
            print_r($json);
        } else {
            print_r($response);
